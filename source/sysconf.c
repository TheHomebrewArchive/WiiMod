...
int SYSCONF_GetTxt(const char *name, char *buf, int length) {
...
s32 SYSCONF_GetVideo(void) {
    ...
    res = SYSCONF_GetTxt("VIDEO", buf, 5);
    if(res < 0) return res;
...
s32 SYSCONF_GetSerialCode(void) {
    int res;
    char buf[4];

    res = SYSCONF_GetTxt("CODE", buf, 4);
    if(res < 0) return res;

    if(!strcmp(buf, "LJF")) return SYSCONF_CODE_JPN;
    if(!strcmp(buf, "LU")) return SYSCONF_CODE_USA;
    if(!strcmp(buf, "LEH")) return SYSCONF_CODE_EURH;
    if(!strcmp(buf, "LEM")) return SYSCONF_CODE_EURM;
    if(!strcmp(buf, "LEF")) return SYSCONF_CODE_EURF;
    if(!strcmp(buf, "LKM")) return SYSCONF_CODE_KOR;
    if(!strcmp(buf, "LAH")) return SYSCONF_CODE_AUS;
    if(!strcmp(buf, "KJF")) return SYSCONF_CODE_JAPK;//uncomfirmed
    if(!strcmp(buf, "KU")) return SYSCONF_CODE_USAK;
    if(!strcmp(buf, "KEH")) return SYSCONF_CODE_EURHK;//uncomfirmed
    if(!strcmp(buf, "KEM")) return SYSCONF_CODE_EURMK;//uncomfirmed
    if(!strcmp(buf, "KEF")) return SYSCONF_CODE_EURFK;//uncomfirmed
    if(!strcmp(buf, "KLM")) return SYSCONF_CODE_KORK;//uncomfirmed
    if(!strcmp(buf, "KAM")) return SYSCONF_CODE_AUSK;//uncomfirmed
    return SYSCONF_EBADVALUE;
}
